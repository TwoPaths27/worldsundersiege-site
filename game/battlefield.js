...
function getBorsTurnAttackState(attacker) {
  const turnKey = `${GameState.turn}:${GameState.activePlayer}`;
  if (attacker?.borsAttackTurnKey !== turnKey) {
    attacker.borsAttackTurnKey = turnKey;
    attacker.borsUnitAttacksThisTurn = 0;
    attacker.borsStrongholdAttackedThisTurn = false;
  }
  return attacker;
}

function canBorsAttackTarget(attacker, isStrongholdAttack = false) {
  if (typeof isSirBors !== "function" || !isSirBors(attacker)) return !attacker.hasAttacked;
  getBorsTurnAttackState(attacker);
  if (isStrongholdAttack) return !attacker.borsStrongholdAttackedThisTurn;
  const itemCount = typeof countItemsEquipped === "function" ? countItemsEquipped(attacker) : (attacker.attachedItems || attacker.equippedItems || []).length;
  const maximumUnitAttacks = Math.max(1, itemCount);
  return Number(attacker.borsUnitAttacksThisTurn || 0) < maximumUnitAttacks;
}

function commitDeclaredAttack(attacker, constructOperator = null, isStrongholdAttack = false) {
  if (isConstruct(attacker)) {
    if (!consumeConstructOperatorAttack(constructOperator, attacker)) {
      return false;
    }
  } else if (typeof isSirBors === "function" && isSirBors(attacker)) {
    getBorsTurnAttackState(attacker);
    if (isStrongholdAttack) attacker.borsStrongholdAttackedThisTurn = true;
    else attacker.borsUnitAttacksThisTurn = Number(attacker.borsUnitAttacksThisTurn || 0) + 1;
    const itemCount = typeof countItemsEquipped === "function" ? countItemsEquipped(attacker) : (attacker.attachedItems || attacker.equippedItems || []).length;
    const maximumUnitAttacks = Math.max(1, itemCount);
    attacker.hasAttacked = attacker.borsStrongholdAttackedThisTurn && attacker.borsUnitAttacksThisTurn >= maximumUnitAttacks;
  } else {
...
async function declareAttack({
  ...
  if (!isConstruct(attacker) && !canBorsAttackTarget(attacker, isStrongholdAttack)) {
    addLog(`${attacker.name} has already used all legal attacks this turn.`);
    renderGame();
    return false;
  }
  ...
  if (!commitDeclaredAttack(attacker, constructOperator, isStrongholdAttack)) {
...
function handleStrongholdClick(targetPlayerId) {
  ...
  if (!isConstruct(attacker) && !canBorsAttackTarget(attacker, true)) {
    addLog(`${attacker.name} has already attacked a Stronghold this turn.`);
    renderGame();
    return;
  }
